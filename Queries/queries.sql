...
-- put the above info into a table to be exported as csv
SELECT COUNT(ce.emp_no), de.dept_no
INTO employee_ct
FROM current_emp as ce
LEFT JOIN dept_employee as de
...
-- check work, output should be same as employee count by department number, but also a table created
SELECT * FROM employee_ct;
...
-- create a retirement report for the sales department
...
-- create a retirement report for both the sales and development teams
SELECT ri.emp_no, ri.first_name, ri.last_name, d.dept_name
INTO saledev_info
FROM retirement_info as ri
INNER JOIN dept_employee AS de
